...
def bpr_triplet_impression_sampler(df,user_id,pos_id,neg_id,price_diff_var,batch_size=1000,shuffle=True):
    '''
    ...
    price_diff: a variable that has the difference in the price of pos_id and neg_id. included in sampler to allow for time-varying differences in prices
...
def build_bpr_model(num_items,num_users,X=None,k=5,usebias=False,pen=0.,include_price=True,neg_price=False):
    '''
    Builds our keras model of BPR
    num_items: number of unique products/items
    num_users: number of unique users
    k: embedding dimensionality
    pen: regularization parameter on user embeddings
    ...
    if include_price == True:
        user_price_vecs = Embedding(input_dim = num_users,output_dim=1,input_length=1,name='price_coef')(user_input) #don't regularize price coef due to differing scale.
...
def export_embeddings(model,product_dict,user_dict,observables=None,usebias=True):
    '''
    ...
    if has_observables:
        product_obs_embeddings = pd.DataFrame(observables, index=products)
